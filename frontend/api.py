...
def get_recommendations(profile: UserProfile):
    ...
    llm_results = llm.generate_batch_project_details(user_input, tasks) if llm else [
        _build_fallback_project_details(task["project_dict"], task["missing_skills"], user_input) for task in tasks
    ]
